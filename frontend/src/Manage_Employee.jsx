...
const Manage_Employee = () => {
  ...
  const handleEdit = (employeeId) => {
    setSelectedServiceId(employeeId);
  ...
  const handleUpdateService = (updatedemployee) => {
    setemployee((prevupdatedemployee) =>
        prevupdatedemployee.map((employee) =>
  ...
  const handleDelete = async (employeeId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await axios.delete(`${apiUrl}/employee/${employeeId}/`);
  ...
  const handleAddService = (employee) => {
    setemployee((prevemployee) => [...prevemployee, employee]); // อัปเดต state ทันทีที่เพิ่ม
  };
  ...
  return (
    <div >
        ...
        <Edit_Employees
            ...
            employeesId={selectedServiceId}
            onUpdateEmployees={handleUpdateService} 
          />
       <div className='container pt-20 mx-auto '>
        
       
      <header className="mb-2 text-start ">
        <h1 className="text-2xl font-bold text-gray-800 p-2 ml-10">MANAGEMENT</h1>
        <h1 className="text-xl font-bold text-gray-800 p-2 ">Barber Information</h1>
...
<table className="w-full  border-gray-600 text-white mt-3 border-separate border-spacing-y-2">
  <thead className="relative -top-3">
    <tr className="bg-white text-black">
      <th className="border-[3px] p-[15px] l border-black border-r-0">No.</th>
      <th className="border-[3px] p-2 border-black border-r-0">Name</th>
      <th className="border-[3px] p-2 border-black border-r-0">Sex</th>
      <th className="border-[3px] p-2 border-black border-r-0">Age</th>
      <th className="border-[3px] p-2 border-black border-r-0">Job Position</th>
      <th className="border-[3px] p-2 border-black border-r-0">Working Status</th>
      <th className="border-[3px] p-2  border-black">Manage</th>
    </tr>
  </thead>
  <tbody>
    {employee.map((employee, index) => (
      <tr key={employee.id} className="bg-black text-white text-center rounded-lg ">
        <td className="border-[1px] rounded-l-md border-black p-2">{index + 1}</td>
        ...
        <td className="border-[1px] p-2  gap-2 justify-center border-black  rounded-r-md">
          ...
          <button className="p-2" onClick={() => handleEdit(employee.id)}>
          ...
          <button onClick={() => handleDelete(employee.id)} className="p-2 rounded text-white">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 448 512">
              <path fill="#db1414" d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"/>
            </svg>
          </button>
          </div>
