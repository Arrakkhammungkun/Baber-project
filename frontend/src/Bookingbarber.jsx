...
const Bookingbarber = () => {
  ...
  useEffect(() => {
    ...
    if (selectedDate && selectedEmployeeId) {
      axios
        .get(`${apiUrl}/bookings/${selectedEmployeeId.id}/${selectedDate.format("YYYY-MM-DD")}/`)
        .then((response) => {
          console.log("Fetched data:", response.data);
  
          setBookedTimes(response.data.booked_times);
          
          const filteredAvailableTimes = response.data.available_times.filter(time => 
            !response.data.booked_times.includes(time)
          );
...
useEffect(() => {
  ...
  axios.get(`${apiUrl}/employee/`)
    .then((response) => {
      ...
      const activeEmployees = response.data.filter((employee) => employee.status === "Active");
  ...
  const handleBooking = async () => {
      ...
      if (isPastTime(selectedTimeText.replace("Selected Time: ", ""), selectedDate.format("YYYY-MM-DD"))) {
        Swal.fire({
          icon: 'error',
  ...
  <div className="fixed inset-0 z-50 bg-gray-500 bg-opacity-50 flex items-center justify-center px-4">
    <div className="bg-white text-black p-6 rounded-xl w-full max-w-2xl">
      <h2 className="text-2xl font-bold mb-4 text-center">สรุปข้อมูลการจอง</h2>
      <hr />
      ...
      <div className="text-base text-black mt-1 p-2">
        ...
        <div className="flex flex-col sm:flex-row items-start gap-4">
          <div className="w-24 h-24 bg-cover bg-center rounded-lg" 
            style={{ backgroundImage: `url(${Service.image_url})` }}>
          </div>
          <div>
            <p><strong>บริการ:</strong> {Service ? `${Service.name} ` : "ยังไม่เลือก"}</p>
            <p className="mt-1"><strong>รายละเอียด:</strong> {Service ? `${Service.description} ` : "ยังไม่เลือก"}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start gap-4 mt-4">
          <div className="w-24 h-24 bg-cover bg-center rounded-lg" 
            style={{ backgroundImage: `url(${selectedEmployeeId.employee_image_url})` }}>
          </div>
          <div>
            <p><strong>ชื่อพนักงาน:</strong> {selectedEmployeeId ? `${selectedEmployeeId.first_name} ${selectedEmployeeId.last_name}` : "ยังไม่เลือก"}</p>
            <p><strong>ชื่อเล่น:</strong> {selectedEmployeeId ? `${selectedEmployeeId.nickname} ` : "ยังไม่เลือก"}</p>

            <div className="flex gap-4 text-base mt-1">
              <p><strong>เพศ:</strong> {selectedEmployeeId ? `${selectedEmployeeId.gender}` : "ยังไม่เลือก"}</p>
              <p><strong>อายุ:</strong> {selectedEmployeeId ? `${calculateAge(selectedEmployeeId.dob)} ปี` : "ยังไม่เลือก"}</p>
            </div>
          </div>
        </div>
        ...
        <p className="text-center text-sm mt-4">**หมายเหตุ** โปรดมาตรงเวลา</p>
      </div>
        ...
        <div className="container mt-24 mx-auto">
          ...
          <div className="flex justify-between w-full">
            ...
            <div className="ml-10 w-full mr-2 p-2">
              ...
              {employees.map((employee, index) => (
                <div 
                  ...
                  ${selectedEmployeeId === employee ? "border-4 border-blue-500 "  : ""}`} // เพิ่ม border สีฟ้าเมื่อเลือกพนักงาน
                >
                  <div className="bg-cover bg-center h-[150px] sm:h-[200px] sm:w-[150px] md:h-[250px] md:w-[200px] rounded-lg" 
            ...
            <div className="w-full md:w-1/2 lg:w-1/3 mx-auto p-2">
              <h1 className="text-xl font-bold text-gray-800 p-2 capitalize text-center">Select Date & Time</h1>
              <hr className="border-black mb-4" />
              {/* Calendar Picker */}
              <div className="bg-black p-4 shadow-md rounded-md text-white">
                <div className="flex justify-between items-center mb-2">
                  <button 
                    className={`text-white hover:text-black ${selectedDate.isSame(today, "month") ? "opacity-50 cursor-not-allowed" : ""}`} 
                    onClick={() => setSelectedDate(prev => prev.subtract(1, "month"))} 
                    disabled={selectedDate.isSame(today, "month")}
                  >
                    &lt;
                  </button>
                  <span className="font-semibold text-lg">{selectedDate.format("MMMM YYYY")}</span>
                ...
                <div className="grid grid-cols-7 text-center">
                  ...
                  {generateCalendar().map((day, index) => {
                    ...
                    return (
                      <button 
                        key={index} 
                        className={`p-2 w-full ${fullDate.isBefore(today, "day") ? "opacity-50 cursor-not-allowed" : fullDate.isSame(selectedDate, "day") ? "bg-blue-500 text-white rounded-full" : "hover:bg-gray-200"}`} 
              ...
              <div className="grid grid-cols-3 gap-2">
                  {availableTimes.map((time, index) => {
                    const isPast = isPastTime(time, selectedDate.format("YYYY-MM-DD"));
                    const isBooked = bookedTimes.includes(time);
                    const isDisabled = isPast || isBooked;

                    const isSelected = time === selectedTimeText.replace("Selected Time: ", "");  // เช็คว่าเป็นเวลาที่เลือกหรือไม่
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-2 rounded-md text-center whitespace-nowrap transition 
                          ...
                          ${!isDisabled ? "bg-black text-white hover:bg-opacity-80" : ""}
                          ${isSelected ? "bg-blue-500 text-black " : ""}  // สีจางๆ เมื่อเลือกแล้ว
                        `}
                        disabled={isDisabled}
