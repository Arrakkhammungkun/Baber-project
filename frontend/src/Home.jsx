  ...
  return (
    <div>
        ...
        <div className="h-full overflow-hidden">
          ...
          <div className="bg-[url(/src/img/welcome.jpg)] bg-cover bg-center h-screen w-full flex items-center justify-center bg-fixed">
            ...
            <div className="relative flex flex-col w-full px-4 sm:px-6 md:px-10 text-white">
              <div className="grid grid-cols-2 gap-2 sm:gap-4 tracking-tighter">
                <h1 className={`text-2xl sm:text-4xl md:text-6xl lg:text-8xl xl:text-[10rem] 2xl:text-[12rem] font-extrabold ${scrolling ? 'animate-slide-left-reverse' : 'animate-slide-right'}`}>
                  WELCOME
                </h1>
                <h1 className={`text-2xl sm:text-4xl md:text-6xl lg:text-8xl xl:text-[10rem] 2xl:text-[12rem] font-extrabold text-end ${scrolling ? 'animate-slide-right-reverse' : 'animate-slide-left'}`}>
                  TO
                </h1>
                <h1 className={`text-2xl sm:text-4xl md:text-6xl lg:text-8xl xl:text-[10rem] 2xl:text-[12rem] font-extrabold ${scrolling ? 'animate-slide-left-reverse' : 'animate-slide-right'}`}>
                  THE
                </h1>
                <h1 className={`text-2xl sm:text-4xl md:text-6xl lg:text-8xl xl:text-[10rem] 2xl:text-[12rem] font-extrabold ${scrolling ? 'animate-slide-right-reverse' : 'animate-slide-left'}`}>
                  STYLEX
                </h1>
              </div>
            </div>
          ...
          <div className="container mx-auto py-6 sm:py-8 md:py-12 px-4 sm:px-6 md:px-8">
            ...
            <div className="flex flex-col md:flex-row-reverse justify-between gap-4 sm:gap-6 md:gap-8 mb-10">
              ...
              <div className="w-full md:w-1/2 text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl">
                STYLEX เกิดขึ้นจากความหลงใหลในศิลปะการสร้างสรรค์ทรงผมและการดูแลเส้นผมอย่างมืออาชีพ เราเริ่มต้นจากร้านเล็กๆ ด้วยความตั้งใจที่จะให้บริการที่เหนือกว่า พร้อมให้คำแนะนำส่วนตัวจากทีมช่างมืออาชีพ ที่พร้อมออกแบบทรงผมที่เหมาะสมและสอดคล้องกับบุคลิกของลูกค้าแต่ละคน เราเชื่อว่าเส้นผมที่ดีไม่เพียงแต่ทำให้คนสวยขึ้น แต่ยังเสริมสร้างความมั่นใจให้กับทุกคนในการเผชิญโลกภายนอก
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 mb-10">
              {[
                { img: '/src/img/salon.png', title: 'hair cut', desc: 'ตัดผมสวย โดนใจ ด้วยช่างมืออาชีพที่ใส่ใจคุณ' },
                { img: '/src/img/razor.png', title: 'shaving', desc: 'เปลี่ยนทุกสัมผัสให้เรียบเนียนในแบบที่คุณมั่นใจ' },
                { img: '/src/img/hair-clipper.png', title: 'trimming', desc: 'เติมความเป๊ะในทุกเส้น สร้างลุคที่ลงตัว' },
                { img: '/src/img/comb.png', title: 'style&care', desc: 'สร้างสไตล์ที่ใช่ พร้อมดูแลเส้นผมให้สุขภาพดี' },
              ].map((service, index) => (
                <div 
                  key={index} 
                ...
                >
                  ...
                  <div className="text-white text-center mt-2 sm:mt-3">
                    <h1 className="text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl font-medium uppercase">{service.title}</h1>
                    <p className="text-[10px] sm:text-xs md:text-sm lg:text-base xl:text-lg font-bold mt-1">{service.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <style jsx>{`
        ...
        .animate-slide-right { animation: slideFromRight 1s ease-out forwards; }
        .animate-slide-left { animation: slideFromLeft 1s ease-out forwards; }
        .animate-slide-left-reverse { animation: slideLeftReverse 1s ease-out forwards; }
        .animate-slide-right-reverse { animation: slideRightReverse 1s ease-out forwards; }

        .gallery-container { width: 100%; overflow: hidden; display: flex; justify-content: center; }
        .slider { width: 100%; overflow: hidden; }
        .slider-track { display: flex; animation: slideInfinite 30s linear infinite alternate; }
        .slider-tracka { display: flex; animation: slideLeftToRight 30s linear infinite alternate; }
        .slider-item { 
          min-width: 200px; 
          margin-right: 8px; 
          flex-shrink: 0; 
        ...
        @media (min-width: 640px) {
          .slider-item { min-width: 250px; height: 180px; margin-right: 12px; }
        }
        @media (min-width: 768px) {
          .slider-item { min-width: 300px; height: 200px; }
        }
        @media (min-width: 1024px) {
          .slider-item { min-width: 350px; height: 250px; }
        }
      `}</style>
    </div>
  );
